...
export default function CheckinPage() {
  ...
  function addHabit() {
    ...
    const h: Habit = { id: crypto.randomUUID(), title: newHabit.trim(), streak: 0, history: {} };
    ...
    <div className="min-h-screen px-6 py-8 sm:px-10 sm:py-12 text-foreground">
      ...
      >
        <div className="grid gap-3 sm:flex sm:items-end">
          ...
          <div className="text-sm opacity-80">Overall today: {allDoneToday ? "Success" : "Pending"}</div>
        </div>
      </HeroCard>
          ...
          return (
            <div key={h.id} className="card-blur rounded-xl p-4">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => removeHabit(h.id)} className="text-red-500 text-sm">Remove</button>
              </div>
              <div className="mt-3 flex items-center gap-3">
                <button
                  ...
                  className={
                    ...
                    (done ? "bg-emerald-500/90 text-white" : "bg-black/5 dark:bg-white/10")
                  }
                >
                  {done ? "Done" : "Mark Done"}
                </button>
